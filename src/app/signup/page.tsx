...
function SignupForm() {
  ...
  useEffect(() => {
    ...
    const handleNativeClick = async (e: MouseEvent) => {
      ...
      // 1. Defend against social media In-App WebViews
      const ua = typeof navigator !== 'undefined' ? navigator.userAgent || "" : "";
      const isInApp = /Instagram|FBAN|FBAV|FB_IAB|TikTok|Twitter|wv\)|WebView|InApp/i.test(ua);
      if (isInApp) {
        toast.error(
          "Google Sign-in is blocked inside social media WebViews. " +
  ...
  const isAnySubmitting = submitting || googleSubmitting;
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
              ...
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-orange-700 mt-1 leading-relaxed">
                    Your browser blocked the secure Google sign-in window. Please click the button again and click **"Allow popups"** in your browser's address bar to log in.
                  </p>
                </div>
              </div>
            </div>
          ...
          <button
            ...
            className="w-full py-3 border border-gray-200 rounded-xl text-gray-700 font-medium flex items-center justify-center gap-3 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 mb-5 cursor-pointer"
          >
            {googleSubmitting ? (
              <svg className="animate-spin h-5 w-5" viewBox="0 0 24 24" fill="none">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 0 18-8V0C5.373 0 0 5.373 0 12h4z" />
              </svg>
            ) : (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={isAnySubmitting}
              className="w-full py-3 bg-gradient-to-r from-orange-500 to-orange-600 text-white font-medium rounded-xl hover:from-orange-600 hover:to-orange-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 shadow-lg shadow-orange-500/25 cursor-pointer"
            >
              {submitting ? (
